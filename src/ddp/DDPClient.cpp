...
int DDPClient::connect(const std::string& uri) {
	...
	std::shared_ptr<sitara::websocket::Connection> connection = getConnection(mConnectionId);

	connection->setOnOpenFn([&](std::shared_ptr<Connection> connection, websocketpp::client<websocketpp::config::asio_client>* client, websocketpp::connection_hdl handle) {
		Json::Value openMsg;
		openMsg["msg"] = "connect";
		if (!mSession.empty()) {
	...
	connection->setOnReceiveFn([&](websocketpp::connection_hdl handle, websocketpp::client<websocketpp::config::asio_client>::message_ptr message) {
		...
		if (receivedMsg["msg"].isNull()) {
			...
			std::cout << "Successfully connected to DDP Server." << std::endl;
			mIsConnected = true;
		}
		else if (receivedMsg["msg"].asString() == "failed") {
			if (receivedMsg["version"].asString() != "1") {
				std::cout << "Server requires an incompatible version of DDP (" << receivedMsg["version"].asString() << ")" << std::endl;
				std::cout << "DDPClient will not try to reconnect." << std::endl;
				close(mConnectionId, websocketpp::close::status::protocol_error);
			}
			else {
				std::cout << "DDPClient failed to connect to DDP Server.  Protocol number (1) is correct; please investigate error further." << std::endl;
			}
			mIsConnected = false;
		}
			...
			if (mOnAddedFn) {
				mOnAddedFn(receivedMsg["collection"].asString(), receivedMsg["id"].asString(), receivedMsg["fields"]);
			}
		}
		else if (receivedMsg["msg"].asString() == "changed") {
			if (mOnChangedFn) {
				mOnChangedFn(receivedMsg["collection"].asString(), receivedMsg["id"].asString(), receivedMsg["fields"]);
			}
		}
		else if (receivedMsg["msg"].asString() == "removed") {
			if (mOnRemovedFn) {
				mOnRemovedFn(receivedMsg["collection"].asString(), receivedMsg["id"].asString());
			}
		}
		else if (receivedMsg["msg"].asString() == "ready") {
			for (Json::Value::ArrayIndex i = 0; i != receivedMsg["subs"].size(); i++) {
				mSubscriptionStatus[receivedMsg["subs"][i].asString()] = ddp::SubscriptionStatus::READY;
			}
		}
			...
			mSubscriptionStatus[receivedMsg["id"].asString()] = ddp::SubscriptionStatus::UNSUBSCRIBED;
		}
	});
...
ddp::SubscriptionStatus DDPClient::getSubscriptionStatus(const std::string& subscriptionName) {
...
std::string DDPClient::subscribe(const std::string& name) {
	...
	mSubscriptionStatus.insert(std::make_pair(subscriptionId, ddp::SubscriptionStatus::CONNECTING));
...
void DDPClient::setOnAddedFn(std::function<void(std::string collection, std::string documentId, Json::Value fields)> fn) {
	mOnAddedFn = fn;
}

void DDPClient::setOnChangedFn(std::function<void(std::string collection, std::string documentId, Json::Value fields)> fn) {
	mOnChangedFn = fn;
}

void DDPClient::setOnRemovedFn(std::function<void(std::string collection, std::string documentId)> fn) {
	mOnRemovedFn = fn;
}
