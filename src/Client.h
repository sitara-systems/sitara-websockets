	...
	namespace websocket {
		...
		public:
			...
			void send(int id, std::string message) {
				...
				if (connection != nullptr) {
					mClient.send(connection->getHandle(), message, websocketpp::frame::opcode::text, errorCode);
					if (errorCode) {
			...
			void send(int id, void const * payload, size_t length) {
				...
				websocketpp::client<websocketpp::config::asio_client>::message_ptr messagePtr;
				if (connection != nullptr) {
					mClient.send(connection->getHandle(), payload, length, websocketpp::frame::opcode::binary, errorCode);
					if (errorCode) {
						std::printf("Error sending message: %s\n", errorCode.message().c_str());
						return;
					}
