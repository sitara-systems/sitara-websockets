...
namespace sitara {
	namespace websocket {
		...
		public:
			...
			std::shared_ptr<Connection> getConnection(websocketpp::connection_hdl handle) {
				std::map<websocketpp::connection_hdl, std::shared_ptr<Connection>>::iterator connection = mHandleMap.find(handle);
		...
		protected:
			...
			std::map<websocketpp::connection_hdl, std::shared_ptr<Connection>, std::owner_less<websocketpp::connection_hdl>> mHandleMap;
		};
	}
}
